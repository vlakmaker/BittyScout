...
# Set project root and add it to the Python path
...
if __name__ == "__main__":
    ...
    config = load_config()
    test_companies = config.get("Personio", [])

    if not test_companies:
        print("⚠️ No Personio companies found in 'config/job_sources.yml' under the 'Personio' key.")
        sys.exit(1)

    print(f"🧪 Testing Personio scraper with companies: {test_companies}")
    scraper = PersonioScraper(company_identifiers=test_companies)
    ...
    cursor.execute("""
        ...
        FROM jobs
        WHERE platform_source='Personio'
    ...
    if results:
        ...
        print("❓ No Personio jobs found in the database after the run.")
